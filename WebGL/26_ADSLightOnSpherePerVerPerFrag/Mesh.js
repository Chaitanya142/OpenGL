...
function Mesh()
{
    ...
    this.allocate=function(numIndices)
    {
        ...
        colors = new Float32Array(iNumIndices * 3 * 4); // 3 is r,g,b and 4 is sizeof float
    ...
    this.addTriangle=function(single_vertex, single_normal, single_texture,single_color)
    {
        ...
        for (i = 0; i < 3; i++)
        {
            for (j = 0; j < numVertices; j++) //for the first ever iteration of 'j', numVertices will be 0 because of it's initialization in the parameterized constructor
            {
                if (isFoundIdentical(verts[j * 3], single_vertex[i][0], diff) &&
                    ...
                    isFoundIdentical(colors[j * 3], single_color[i][0], diff) &&
                    isFoundIdentical(colors[(j * 3) + 1], single_color[i][1], diff) &&
                    isFoundIdentical(colors[(j * 3) + 2], single_color[i][2], diff) &&
            ...
            if (j == numVertices && numVertices < maxElements && numElements < maxElements)
            {
                ...
                colors[numVertices * 3] = single_color[i][0];
                colors[(numVertices * 3) + 1] = single_color[i][1];
                colors[(numVertices * 3) + 2] = single_color[i][2];
