...
[[self openGLContext]setValues:&swapInt forParameter:NSOpenGLCPSwapInterval];
        ...
        if (iInfoLogLength > 0) {
            ...
            if (szInfoLog != NULL) {
                ...
                glGetShaderInfoLog(gTessellationControlShaderObject,
                    ...
                    &written,
                    szInfoLog);
                fprintf(gpFile, "Tesselation Shader Compilation Log %s\n", szInfoLog);
                free(szInfoLog);
                [self release];
                [NSApp terminate:self];
            }
        ...
        if (iInfoLogLength > 0) {
            ...
            if (szInfoLog != NULL) {
                ...
                glGetShaderInfoLog(gTessellationEvaluationShaderObject,
                    ...
                    &written,
                    szInfoLog);
                fprintf(gpFile, "Tesselation Eval Shader Compilation Log %s\n", szInfoLog);
                free(szInfoLog);
                [self release];
                [NSApp terminate:self];
            }
    ...
    glVertexAttribPointer(AMC_ATTRIBUTE_POSITION,2,GL_FLOAT,
                            GL_FALSE,0,NULL);
...
glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    ...
    vmath::vec4 colorVec = vmath::vec4(1.0f,1.0f,0.0f,1.0f);
