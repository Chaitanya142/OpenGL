...
// ctor
///////////////////////////////////////////////////////////////////////////////
Sphere::Sphere() : interleavedStride(32)
{
      pFile = fopen ("/Users/apple/Documents/Mac OSX/17_DynamicIndia/Log1.txt","w");
...
void Sphere::drawWithColor() const
{
  
    /*  if (pFile!=NULL)
    {
        unsigned int temp=(unsigned int)indices.size();
    ...
    //GLint count = (GLint)MySphere.getIndicesCount();
        //fprintf(GMD_gpFile, "count %d",count);
    
    //const unsigned int* pos = MySphere.getIndices();
    
      // prints the vector
    //for (unsigned int i = 0; i < MySphere.getIndicesCount(); ++i)
      //    fprintf(GMD_gpFile, "%u ",*pos++);	//glActiveTexture(GL_TEXTURE0);
    ...
    //fprintf(pFile, "count %d",AMC_ATTRIBUTE_COLOR);
    
    glVertexAttrib3f(AMC_ATTRIBUTE_COLOR,
		1.0f, 1.0f, 1.0f);

    //glDrawArrays(GL_TRIANGLE_FAN, 0, sphere.getIndicesCount());
...
void Sphere::buildVerticesSmooth()
{
    ...
    //fprintf(pFile,"PI %f StackStep %f SectorAngle %f \n",PI,sectorStep,stackStep);
	...
	for (int i = 0; i <= stackCount; ++i)
	{
       ...
       // fprintf(pFile,"stackAngle %f xy %f z %f",stackAngle,xy,z);
		...
		for (int j = 0; j <= sectorCount; ++j)
		{
           ...
           // fprintf(pFile,"Sector Angle %f ",sectorAngle);
            
			// vertex position
			x = (xy * cosf(sectorAngle)) + cx;             // r * cos(u) * cos(v)
			y = (xy * sinf(sectorAngle)) + cy;             // r * cos(u) * sin(v)
			addVertex(x, y, z);
...
void Sphere::buildInterleavedVertices()
{
	...
	for (i = 0, j = 0; i < count; i += 3, j += 2)
	{
        ...
        interleavedVertices.push_back(colorCoords[i]);
        interleavedVertices.push_back(colorCoords[i + 1]);
        interleavedVertices.push_back(colorCoords[i + 2]);
    ...
    //for(i=0;i<interleavedVertices.size();i++)
      //  fprintf(pFile,"%f ",interleavedVertices.at(i));
...
void Sphere::createVAO() {
	...
	glBufferData(GL_ARRAY_BUFFER,
		...
		//interleavedVertices,
		sizeof(interleavedVertices[0]) * interleavedVertices.size(),
		reinterpret_cast<GLvoid*>(&interleavedVertices[0]),
		GL_STATIC_DRAW);
    
    //fprintf(pFile, "size %d",interleavedVertices.size());
...
void Sphere::addColor(float r, float g, float b)
{
    colorCoords.push_back(r);
    colorCoords.push_back(g);
    colorCoords.push_back(b);
}
///////////////////////////////////////////////////////////////////////////////
...
void Sphere::setTextureColor(float R, float G, float B) {
	this->R = R;
	this->G = G;
	this->B = B;
}
...
void Sphere::setCenter(float cx, float cy) {
	this->cx = cx;
	this->cy = cy;
}
