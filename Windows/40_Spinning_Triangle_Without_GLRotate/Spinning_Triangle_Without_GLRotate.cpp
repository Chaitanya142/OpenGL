...
void Display(void) {
	...
	static GLfloat angle_Trianlge = 0.0f;
	void DrawFilledTriangle(float offset);
	...
	rotation_matrix[0] = cos(angle_Trianlge);
	rotation_matrix[1] = 0.0f;
	rotation_matrix[2] = -sin(angle_Trianlge);
	...
	rotation_matrix[8] = sin(angle_Trianlge);
	rotation_matrix[9] = 0.0f;
	rotation_matrix[10] = cos(angle_Trianlge);
	...
	DrawFilledTriangle(1.0f);
	
	SwapBuffers(ghdc);

	angle_Trianlge = angle_Trianlge + 0.001f;
