...
void Display(void) {
	...
	static GLfloat circleProjectionX = -2.60f;
	static GLfloat circleProjectionY = -2.60f;

	static GLfloat triangleProjectionX = 2.60f;
	static GLfloat triangleProjectionY = -2.60f;

	static GLfloat lineProjectionX = 0.00f;
	static GLfloat lineProjectionY = 2.60f;
	...
	if (stage > 0) {
		glLoadIdentity();
		glTranslatef(triangleProjectionX, triangleProjectionY, -6.0f);
	...
	if (stage > 1) {
		glLoadIdentity();
		glTranslatef(circleProjectionX, circleProjectionY, -6.0f);
		glRotatef(angle, 0.0f, 1.0f, 0.0f);
		DrawInCircle(0.0f, 0.6f, 0.0f,
			-0.6f, -0.6f, 0.0f,
			0.6f, -0.6f, 0.0f
		);
	}
	...
	if (stage > 2) {
		glLoadIdentity();
		glTranslatef(lineProjectionX, lineProjectionY, -6.0f);
		glRotatef(angle, 0.0f, 1.0f, 0.0f);
		DrawLine(0.0f, 0.6f, 0.0f,
			0.0f, -0.6f, 0.0f);
	}
	...
	if (stage == 1) {
		if (triangleProjectionX > 0.0f)
			triangleProjectionX = triangleProjectionX - 0.0008f;
		if (triangleProjectionY < 0.0f)
			triangleProjectionY = triangleProjectionY + 0.0008f;

		if (triangleProjectionX <= 0.0f && triangleProjectionY >= 0.0f)
			stage = 2;
	}

	if (stage == 2) {
		if (circleProjectionX < 0.0f)
			circleProjectionX = circleProjectionX + 0.0008f;
		if (circleProjectionY < 0.0f)
			circleProjectionY = circleProjectionY + 0.0008f;

		if (circleProjectionX >= 0.0 && circleProjectionY >= 0.0f)
			stage = 3;
	}

	if (stage == 3) {
		if (lineProjectionY > 0.0f)
			lineProjectionY = lineProjectionY - 0.0008f;
		if (lineProjectionY < 0.0f)
			stage = 4;
	}
