...
ID3D11Buffer *gpID3D11Buffer_VertexBufferNormalPyramid = NULL;
...
HRESULT initialize() {
	...
	bufferDescNormal.ByteWidth = sizeof(float)*ARRAYSIZE(normals_Pyramid);
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDescNormal, NULL,
		&gpID3D11Buffer_VertexBufferNormalPyramid);
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBufferNormalPyramid, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, normals_Pyramid, sizeof(normals_Pyramid));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBufferNormalPyramid, NULL);
...
void display(void)
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBufferNormalPyramid,
		&stride, &offset);
	...
	//rotationMatrix = XMMatrixRotationRollPitchYaw(anglePyramid, anglePyramid, anglePyramid);
	...
	if (gbLight) {
		...
		constantBuffer.LightPosition[1] = XMVectorSet(lights[1].lightPosition[1], lights[1].lightPosition[1], lights[1].lightPosition[2], lights[1].lightPosition[3]);
...
void update() {
	if (gb_Animation) {
		if (anglePyramid < 360)
			anglePyramid = anglePyramid + 0.0005;
		else
			anglePyramid = 0.0f;
	}
}
