...
HRESULT initialize() {
	...
	bufferDescColor.ByteWidth = sizeof(float)*ARRAYSIZE(colors);
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDescColor, NULL,
		&gpID3D11Buffer_VertexBufferColor);
...
void display(void)
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBufferColor,
		&stride, &offset);
