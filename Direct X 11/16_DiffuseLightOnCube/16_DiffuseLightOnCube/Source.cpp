...
ID3D11Buffer *gpID3D11Buffer_VertexBufferNormalCube = NULL;
...
HRESULT initialize() {
	...
	const char *vertexShaderSouceCode =
		...
		"output.diffused_light=ld*kd*max(dot(s,tnorm),0.0f);"\
		"}"\
		"else"\
		"{"\
		"output.diffused_light=float4(1.0f,1.0f,1.0f,1.0f);"\
		"}"\
	...
	const char * pixelShaderSourceCode =
		"float4 main(float4 pos:SV_POSITION,float4 diffused_light : COLOR) : SV_TARGET"\
		"{"\
		"float4 color=diffused_light;"\
		"return(color);"\
		"}";
	...
	bufferDescNormal.ByteWidth = sizeof(float)*ARRAYSIZE(normals_cube);
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDescNormal, NULL,
		&gpID3D11Buffer_VertexBufferNormalCube);
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBufferNormalCube, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, normals_cube, sizeof(normals_cube));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBufferNormalCube, NULL);
...
void display(void)
{
	...
	XMMATRIX mvMatrix = worldMatrix * viewMatrix;
	...
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBufferNormalCube,
		&stride, &offset);
	...
	mvMatrix = worldMatrix * viewMatrix;


	//constantBuffer;
	if (gbLight) {
		constantBuffer.WorldViewMatrix = mvMatrix;
		constantBuffer.ProjectionMatrix = gPerspectiveProjectionMatrix;
		constantBuffer.LightPosition = XMVectorSet(0.0f, 0.0, -2.0, 1.0f);
		constantBuffer.Kd = XMVectorSet(0.5f, 0.5, 0.5f, 1.0f);
...
void uninitialize()
{
	...
	ReleaseResource((IUnknown *)gpID3D11Buffer_VertexBufferNormalCube);
