...
HRESULT initialize() {
	...
	const char *geometryShaderSouceCode =
		"cbuffer ConstantBuffer"\
		"{"\
		"float4x4 worldViewProjectionMatrix;"\
		"}"\
	...
	hr = D3DCompile(geometryShaderSouceCode,
		lstrlenA(geometryShaderSouceCode) + 1,
		"GS",
		NULL,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,
		"main",
	...
	if (FAILED(hr))
	{
		...
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "D3DCompile() Failed For Geomtery Shader : %s.\n", (char*)pID3DBlob_Error->GetBufferPointer());
			fclose(gpFile);
			//WriteToFile("D3DCompile() Failed For Vertex Shader : %s.\n"+(char*)pID3DBlob_Error->GetBufferPointer());
			pID3DBlob_Error->Release();
			pID3DBlob_Error = NULL;
		...
		WriteToFile("D3DCompile() Succeeded For Geomtery Shader.\n");
	}
	...
	if (FAILED(hr))
	{
		WriteToFile("ID3D11Device::CreateGeometryShader() Failed .\n");
		return(hr);
	}
	else {
		WriteToFile("ID3D11Device::CreateGeometryShader() Succeeded.\n");
	}
