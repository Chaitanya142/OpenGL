...
ID3D11Buffer *gpID3D11Buffer_VertexBufferTexturePyramid = NULL;

ID3D11Buffer *gpID3D11Buffer_VertexBufferPositionCube = NULL;
ID3D11Buffer *gpID3D11Buffer_VertexBufferTextureCube = NULL;
...
HRESULT initialize() {
	...
	bufferDescColor.ByteWidth = sizeof(float)*ARRAYSIZE(texcoord_Pyramid);
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDescColor, NULL,
		&gpID3D11Buffer_VertexBufferTexturePyramid);
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBufferTexturePyramid, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, texcoord_Pyramid, sizeof(texcoord_Pyramid));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBufferTexturePyramid, NULL);
	...
	bufferDescColor.Usage = D3D11_USAGE_DYNAMIC;
	bufferDescColor.ByteWidth = sizeof(float)*ARRAYSIZE(texcoord_Cube);
	bufferDescColor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescColor.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDescColor, NULL,
		&gpID3D11Buffer_VertexBufferTextureCube);
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBufferTextureCube, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, texcoord_Cube, sizeof(texcoord_Cube));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBufferTextureCube, NULL);
	...
	if (FAILED(hr)) {
		WriteToFile("LoadD3DTexture() Failed For Pyramid Texture.\n");
		return(hr);
	}
	else {
		WriteToFile("LoadD3DTexture() Succeeded For Pyramid Texture.\n");
	}
	...
	if (FAILED(hr)) {
		WriteToFile("ID3D11Device::CreateSamplerState() Failed For Pyramid Texture.\n");
		return(hr);
	}
	else {
		WriteToFile("ID3D11Device::CreateSamplerState() Succeeded For Pyramid Texture.\n");
	...
	if (FAILED(hr)) {
		WriteToFile("LoadD3DTexture() Failed For Cube Texture.\n");
		return(hr);
	}
	else {
		WriteToFile("LoadD3DTexture() Succeeded For Cube Texture.\n");
	}
	...
	if (FAILED(hr)) {
		WriteToFile("ID3D11Device::CreateSamplerState() Failed For Cube Texture.\n");
		return(hr);
	}
	else {
		WriteToFile("ID3D11Device::CreateSamplerState() Succeeded For Cube Texture.\n");
	}
...
void display(void)
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBufferTexturePyramid,
		&stride, &offset);
	...
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBufferTextureCube,
		&stride, &offset);
...
void uninitialize()
{
	...
	ReleaseResource((IUnknown *)gpID3D11Buffer_VertexBufferTexturePyramid);
	ReleaseResource((IUnknown *)gpID3D11Buffer_VertexBufferPositionCube);
	ReleaseResource((IUnknown *)gpID3D11Buffer_VertexBufferTextureCube);
