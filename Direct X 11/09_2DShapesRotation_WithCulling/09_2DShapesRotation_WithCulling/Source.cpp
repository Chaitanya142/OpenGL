...
ID3D11Buffer *gpID3D11Buffer_VertexBufferPositionTriangle = NULL;
ID3D11Buffer *gpID3D11Buffer_VertexBufferColorTriangle = NULL;

ID3D11Buffer *gpID3D11Buffer_VertexBufferPositionRectangle = NULL;
ID3D11Buffer *gpID3D11Buffer_VertexBufferColorRectangle = NULL;
...
HRESULT initialize() {
	...
	bufferDescPosition.ByteWidth = sizeof(float)*ARRAYSIZE(vertices_triangle);
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDescPosition, NULL,
		&gpID3D11Buffer_VertexBufferPositionTriangle);
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBufferPositionTriangle, NULL,
		D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, vertices_triangle, sizeof(vertices_triangle));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBufferPositionTriangle, NULL);
	...
	bufferDescColor.ByteWidth = sizeof(float)*ARRAYSIZE(colors_triangle);
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDescColor, NULL,
		&gpID3D11Buffer_VertexBufferColorTriangle);
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBufferColorTriangle, NULL,
		D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, colors_triangle, sizeof(colors_triangle));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBufferColorTriangle, NULL);
	...
	mappedSubresource;
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBufferPositionRectangle, NULL,
		D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	...
	bufferDescColor.Usage = D3D11_USAGE_DYNAMIC;
	bufferDescColor.ByteWidth = sizeof(float)*ARRAYSIZE(colors_rectangle);
	bufferDescColor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescColor.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDescColor, NULL,
		&gpID3D11Buffer_VertexBufferColorRectangle);
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBufferColorRectangle, NULL,
		D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, colors_rectangle, sizeof(colors_rectangle));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBufferColorRectangle, NULL);
...
void display(void)
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBufferPositionTriangle,
		&stride, &offset);

	stride = sizeof(float) * 3;
	offset = 0;

	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBufferColorTriangle,
		&stride, &offset);
	...
	rotationMatrix = XMMatrixRotationY(angleRotation);
	...
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBufferColorRectangle,
		&stride, &offset);
...
void update() {
	angleRotation = angleRotation + 0.001;
}
...
void uninitialize()
{
	...
	ReleaseResource((IUnknown *)gpID3D11Buffer_VertexBufferPositionTriangle);
	ReleaseResource((IUnknown *)gpID3D11Buffer_VertexBufferColorTriangle);
	ReleaseResource((IUnknown *)gpID3D11Buffer_VertexBufferPositionRectangle);
	ReleaseResource((IUnknown *)gpID3D11Buffer_VertexBufferColorRectangle);
